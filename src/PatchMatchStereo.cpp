...
bool PatchMatchStereo::Match(const uint8* img_left, const uint8* img_right, float32* disp_left)
{
	...
	if (img_left == nullptr || img_right == nullptr) return false;
	...
	// 计算左右视图的灰度图(将彩色图转换为灰度图)
	ComputeGray();

	// 计算左右视图的梯度图（基于3x3的sobel核）
	ComputeGradient();
	...
	for (int k = 0; k < 2; k++) {
		...
		for (sint32 y = 0; y < height; y++) {
			for (sint32 x = 0; x < width; x++) {
				...
				// 随机视差值
                // 将右视图的d设置为负，这样的设定是为了左右视图分别当参考视图进行传播时，都满足d = x1-x2，这样可以反复调用传播函数
...
void PatchMatchStereo::ComputeGradient() const
{
	...
	// Sobel梯度算子
	for (sint32 n = 0; n < 2; n++) {
		auto* gray = (n == 0) ? gray_left_ : gray_right_;   // 左右视图的灰度图像
		auto* grad = (n == 0) ? grad_left_ : grad_right_;   // 左右图像的梯度图像
		...
		for (sint32 y = 0; y < height; y++) {
			for (sint32 x = 0; x < width; x++) {
				...
				if (col_right >= 0 && col_right < width) {
					...
					// 在本代码里，左右视图的视差值符号相反，相减就是用加法就可以了
...
// PMS的填充是把所有无效像素都当做遮挡像素来看待的
// 遮挡区往往是背景区域被前景遮挡而造成的视差错误，而背景是离相机更远的区域，所以视差是较小值(视差小，深度大)。
// 基于此假设，PMS的做法是：对于无效像素p，往其左边和右边分别搜索到第一个有效视差像素，将俩像素的平面fl fr分配给像素，并计算视差值，选择较小的那个视差值。
