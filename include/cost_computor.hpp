...
/**
 * \brief 代价计算器基类
 * 用代价聚合最小化来找到对于p点而言最佳的视差平面f
 * m(p,f) = sum_{q∈Wp} w(p,q)*ρ(q, q')
 */
class CostComputer {
public:
	/** \brief 代价计算器默认构造 */
...
public:
	 ...
	 */
	CostComputerPMS(const uint8* img_left, const uint8* img_right, const PGradient* grad_left,
                    const PGradient* grad_right, const sint32& width,
                    const sint32& height, const sint32& patch_size,
    ...
    /**
	 * \brief 计算左影像p点视差平面为f时的聚合代价值
	 * 聚合代价是以(x,y)为中心的窗口内所有像素在同一个视差平面下的代价加权和
	 * m(p,f) = sum_{q∈Wp} w(p,q)*ρ(q, q')
	 * \param x		p点x坐标
	 * \param y 	p点y坐标
	 * \param p		平面参数
    ...
    {
        const auto pat = patch_size_ / 2;   // 窗口大小的一半
        const auto& col_p = GetColor(img_left_, x, y);  // 获取p点的颜色
        float32 cost = 0.0f;
        // 遍历以(x, y)为中心的窗口
        for (sint32 r = -pat; r <= pat; r++) {
            ...
            for (sint32 c = -pat; c <= pat; c++) {
                ...
                // 计算窗口内该点q在视差平面p内的视差值,知道这个视差值就可以用compute计算得到
...
#ifdef USE_FAST_EXP
                const auto w = fast_exp(double(-dc / gamma_));  // 权重参数w(p, q) = exp(-|Ip-Iq|/γ)
#else
	...
	/**
	 * \brief 计算左影像p点视差为d时的代价值
	 * ρ(q,q) = (1-α)*min(|Iq-Iq'|, col_tau) + α*min(|▽Iq-▽Iq'|, grad_tau),其中q‘是q点视差为d时在右视图的对应点
	 * \param x		p点x坐标
	 * \param y		p点y坐标
	 * \param d		视差值
	 * \return 代价值
	...
	inline float32 Compute(const sint32& x, const sint32& y, const float32& d) override
	{
        ...
        /**========= 计算颜色空间距离 =========**/
		const auto col_p = GetColor(img_left_, x, y);   // 获取p点的RGB颜色数值
		const auto col_q = GetColor(img_right_, xr, y); // q点的x坐标因为不是整数，因此要通过线性插值来求得
        // 计算颜色空间的L1距离
		...
		const auto grad_q = GetGradient(grad_right_, xr, y);    //由于q点坐标非整数，线性内插得到q点的梯度值
	...
	/**
	 * \brief 计算左影像p点视差为d时的代价值
	 *  ρ(q,q) = (1-α)*min(|Iq-Iq'|, col_tau) + α*min(|▽Iq-▽Iq'|, grad_tau)
	 *  其中q‘是q点视差为d时在右视图的对应点，已知q点的颜色和梯度的前提下，计算q和q'两个像素的匹配代价
	 * \param col_p		p点颜色值
	 * \param grad_p	p点梯度值
	 * \param x			p点x坐标
	 * \param y			p点y坐标
	...
	* \return 像素(x,y)的颜色值
	*/
	inline PVector3f GetColor(const uint8* img_data, const float32& x,const sint32& y) const    // 多态，x坐标为非整数的话，要通过线性插值得到该点的颜色值
	{
		float32 col[3];
