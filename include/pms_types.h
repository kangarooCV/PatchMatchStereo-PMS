...
struct DisparityPlane {
	 ...
	 * 左右视图满足：(1) xr = xl - d_p; (2) yr = yl; (3) d_p = -d_q 视差符号相反(本代码左视差为正值，右视差为负值),
	...
	DisparityPlane to_another_view(const sint32& x, const sint32& y) const
	{
        ...
        // a2 = -a1
        // b2 = -b1
        // c2 = -c1 - a1*d1 => c1 = -c2 + a2*d1
        // d2 = -al*xr -bl*yr -(cl + al*dl) => d2 = -al(xl-dl) -bl*yl -(cl+al*dl)=-al*xl -bl*yl -cl = -d1
